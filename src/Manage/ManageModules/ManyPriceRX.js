...
function ManyPriceRX(props){
    const [price,setPrice] = useState(0)
    const [error,setError] = useState('')
    const changeAllValue=(priceKind)=>{
        if(props.content.manufacture.length!==1)
        {
        ...
        if(price.length>4){
            ...
            body:JSON.stringify({sku:props.content.manData.map(item=>item.sku),
        ...
        fetch(env.siteApi+"/order/manufacture/price",postOptions)
        .then(res => res.json())
        .then(
            (result) => {
            //console.log(result);
