...
function StockFaktorSingle(props){
    ...
    const [stockGuranteeName,setStockGuranteeName] = useState('')
        ...
        const postOptions={
            ...
            headers: { 
              ...
              body:JSON.stringify({...orderData,
                ...
                stockService:faktorS,
                stockGurantee:JSON.stringify(faktorS).includes('گارانت')?"request":"",
                stockGuranteeName:stockGuranteeName,
    ...
    return(<>
        ...
        {props.singleOrder?props.showSingle?<SingleOrder 
          setFaktorS={setFaktorS} faktorS={faktorS} setStockGuranteeName={setStockGuranteeName}
          brand={props.singleOrder}/>:
            <button>درخواست سفارش تکی</button>:<></>}
        {props.singleOrder?<div className="orderContinue" style={{float: "none"}}>
            {faktor&&faktor.length?<>
              {(token.access==="manager"||token.access==="sale")?<></>:
                <ButtonLoaderSimple className="orderBtn"
                style={{float: "none"}} action={()=>updateOrderStatus(0)} popTitle="ثبت نهایی سفارش"
                    completed={()=>{console.log("completed")}}//document.location.href="/profile#orders"}
            ...
            {((token.access==="manager"||token.access==="sale")&&
            faktor&&faktor.length&&selectedCustomer&&props.singleOrder)?
            <ButtonLoaderSimple className={"orderBtn"} action={()=>updateOrderStatus(1)}
                style={{float: "none"}} //completed={()=>document.location.reload()}
                popText="در صورت تایید سفارش برای شما فاکتور نهایی صادر میشود
                 و در صورت انصراف میتوانید درخواست خود را اصلاح نمایید
                "   title={"ثبت مستقیم سفارش"}/>:''}
        </div>:<small className="errorBtn">سفارش تکی نیست</small>}
        
            
        <small className="errorBtn">{error.message}</small>
        {0&&alertShow.show?<SimpleAlert data={{text:"سفارش با موفقیت ثبت شد"}}
           setAlertShow={setAlertShow} nocancel={1}/>:<></>}
        </>
    )
}
export default StockFaktorSingle