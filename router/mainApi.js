...
exports.customersApi=async(req,res)=>{
    ...
    try{
        ...
        const userData = await user.find(data.customer?{$or:[
            ...
            .find(data.access&&{access:data.access}).sort({date:-1})
        ...
        const userAll = await user.find(data.customer?{$or:[
            ...
            {cName:new RegExp('.*' + data.customer + '.*')}]}:{})
            .find(data.group&&{group:data.group})
            .find(data.access&&{access:data.access})
        ...
        const userAccess = [...new Set(totalUser.map(item=>item.access))];

        res.json({customers:userData,userData:userAll,size:userAll.length,
            userGroup:userGroup,userAccess:userAccess})
    }
    catch(error){
        res.status(500).json({message: error.message})
    }
} 
exports.creditApi=async(req,res)=>{
    //console.log("CustomerApi")
    var pageSize = req.body.pageSize?req.body.pageSize:"10";
    try{
        const data={
        customer:req.body.customer,
        currentOrder:parseInt(req.body.currentOrder&&
            req.body.currentOrder.toString().replace(/,/g,''))
        
    } 
        //const userCount = (await user.find()).length;
        const userData = await user.findOne({_id:data.customer})
        //console.log(userData)
        if(!userData){
        ...
        if(!userData.cCode){
            res.status(500).json({error: "user Code Error"})
            return
        }
        const userHesabData = await HesabFaApiCall("/contact/get",
        {code:userData.cCode.replace('MGM','')})
        const userOrderData = await RXSchema.find({userId:data.customer,
            hesabfaFaktor:{ $exists : false },
            status:{$ne:"completed"}});
        const currCredit = userHesabData.Result&&userHesabData.Result.ContactCredit;
        const Liability = userHesabData.Result&&userHesabData.Result.Liability;
        var unProcess = 0;
        for(var i=0;i<userOrderData.length;i++){
            unProcess+= parsIntFunc(userOrderData[i].totalPrice)
        }
        const totalCredit = currCredit-(Liability+unProcess+data.currentOrder);
        
        res.json({hesabfa:userHesabData,orders:userOrderData,
        data:{currCredit,Liability,unProcess,credit:totalCredit,
            userGroup:userData.group,
            creditStatus:(totalCredit<0&&userData.group==="reyham")?"red":"green"}})
    } 
    catch(error){
...
exports.cCodeFindApi=async(req,res)=>{
    //console.log("CustomerApi")
    try{ 
        //const userCount = (await user.find()).length;
        const userData = await user.find(
            {cCode:new RegExp('.*' + req.body.cCode + '$')})
        
        res.json({customers:userData})
    }
    catch(error){
        res.status(500).json({message: error.message})
    }
